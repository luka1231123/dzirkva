...
QUERY_TIPS = [
    "დაწერეთ ისე, როგორც ადამიანს ჰკითხავდით: „როგორ გავაკეთოთ ღვინო“. ძირკვა თავად ეძებს „ღვინის გაკეთებას“ "
    "და სიტყვის სხვა ფორმებს.",
    "ორი ან სამი მთავარი სიტყვა საკმარისია. „რა“, „არის“, „და“ და მსგავსი სიტყვები ძიებაში არ ითვლება.",
    "ქართული შრიფტი არ გაქვთ? დაწერეთ ლათინური ასოებით: kartuli ena. შეცდომას ძირკვა ასწორებს ან გკითხავთ, "
    "ხომ არ გულისხმობდით სხვა სიტყვას.",
    "„დისერტაცია“ ან „სტატია“ სამეცნიერო ნაშრომებს წინ წამოწევს, „... რას ნიშნავს“ ლექსიკონის განმარტებას "
    "აჩვენებს, საიტის სახელი (მაგთი, ფეისბუქი) კი ამ საიტის გვერდებს.",
    "ამინდისთვის, რუკისთვის ან ყიდვისთვის დიდი საძიებო სისტემები უფრო მოსახერხებელია. ძირკვა სწავლისთვის და "
    "ქართული ვების აღმოსაჩენად არის გაკეთებული.",
]
DEEP_WHEN = [
    "ჩვეულებრივმა ძიებამ ცოტა ან უადგილო შედეგი მოგცათ.",
    "ეძებთ იშვიათ სიტყვას, სლენგს ან ძველ ტექსტს, მაგალითად „ჩამოლაბორანტება“.",
    "თემას სწავლობთ და გინდათ ნახოთ ყველაფერი, რაც მასზე ქართულად წერია.",
]
GROW = [
...
def home_page() -> str:
    """The start page: why big engines fail Georgian and what dzirkva does, how it tells a person's text,
    how to write a query, when to use ამოძირკვა, finds of the day."""
    ...
    items = lambda xs: "<ul class=list>" + "".join(f"<li>{escape(x)}</li>" for x in xs) + "</ul>"
    ...
    return (f"<div class='ans hero'><div class=cap>{cap('ქართული ვების საძიებო')}</div>"
            "<span class=t>საძიებო, რომელიც ქართულად კითხულობს</span>"
            "<p class=lead>Google და სხვა დიდი საძიებო სისტემები მსოფლიოს ყველა ენაზე ეძებენ. ქართული გვერდი მათთვის "
            "ერთი წვეთია: სიას დიდი პორტალები, ერთი და იგივე ახალი ამბის ასლები და მაღაზიები ავსებს, ბლოგი, ფორუმი, "
            "სამეცნიერო ნაშრომი თუ ბიბლიოთეკის წიგნი კი ქვემოთ რჩება. ქართულ გრამატიკასაც ცუდად იცნობენ: ხშირად "
            "ეძებენ ზუსტად იმ ფორმას, რაც დაწერეთ, და „როგორ გავაკეთოთ ღვინო“ ვერ პოულობს გვერდს "
            "„ღვინის გაკეთებაზე“.</p>"
            "<p class=lead>ძირკვა მხოლოდ ქართულ ვებს ეძებს და შეკითხვას ქართული გრამატიკით კითხულობს: ეძებს სიტყვის "
            "ყველა ფორმას და იმ სიტყვებს, რომლებითაც პასუხი იწერება. ეკითხება რამდენიმე საძიებო სისტემას და "
            "საკუთარ ინდექსებს, სიის თავში კი ადამიანების დაწერილ ტექსტს აყენებს. რეკლამა არ არის და პასუხს "
            f"ხელოვნური ინტელექტი არ წერს.</p><p class=facts>{index}</p></div>"
            + _block("როგორ დავწეროთ შეკითხვა", items(QUERY_TIPS))
            + _block("როდის გამოვიყენოთ ამოძირკვა",
                     "<p>ღილაკი „ამოძირკვა“ იმავე შეკითხვას სამჯერ უფრო ღრმად ეძებს: საძიებო სისტემებში მეტ გვერდს "
                     "კითხულობს, სიტყვის მეტ ფორმას და პასუხის მეტ სიტყვას ცდის, ჩვენს ინდექსებში სამჯერ მეტ შედეგს "
                     "იღებს. ის უფრო ნელია. გამოიყენეთ, როცა:</p>" + items(DEEP_WHEN)
                     + "<p>ყოველდღიური კითხვისთვის ჩვეულებრივი ძებნა საკმარისია.</p>")
            + _block("როგორ ცნობს ადამიანის ტექსტს",
                     ...
                     "ადამიანმა დაწერა ან შეარჩია, და ასეთ გვერდებს წინ აყენებს. შედეგებთან ეს ნიშნები ასე ჩანს:</p>"
                     f"<ul class='list signs'>{signs}</ul>"
                     "<p>ერთი ტექსტი ბევრ საიტზე ერთ შედეგად ჩანს, ერთ საიტს სიაში ცოტა ადგილი აქვს, ონლაინ-მაღაზიებს "
                     "კი ჩვენი ქრაულერი არ აგროვებს.</p>"
