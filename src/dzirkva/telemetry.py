...
def stats(evs: list[dict]) -> dict:
    ...
    return {
        ...
        "intents": Counter(e.get("intent") or "—" for e in fresh).most_common(15),
        "types": Counter(e.get("type") or "—" for e in fresh).most_common(),
        "features": {
        ...
        "langs": Counter(e.get("lang") or "—" for e in evs if e["kind"] in ("search", "home")).most_common(8),
